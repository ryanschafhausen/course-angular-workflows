...
describe('HackerListComponent', () => {
  ...
  describe('initial display', () => {
    it('makes a call to api.getHackers', () => {
      spyOn(api, 'getHackers').and.returnValue(observableOf([]));
    ...
    it(
      ...
      async(() => {
        spyOn(api, 'getHackers').and.returnValue(observableOf(mockHackers));

        fixture.detectChanges();

        fixture.whenStable().then(() => {
          expect(component.hackers).toEqual(mockHackers as any);
        });
      })
    );

    it(
      'sets initial data (using fakeAsync)',
      fakeAsync(() => {
        spyOn(api, 'getHackers').and.returnValue(observableOf(mockHackers));
    ...
    it(
      'renders correct number of tbody rows',
      fakeAsync(() => {
        spyOn(api, 'getHackers').and.returnValue(observableOf(mockHackers));
        ...
        const hackerRows = fixture.debugElement.queryAll(
          By.css('tr[app-hacker]')
        );
  ...
  describe('click on hacker', () => {
    it(
      ...
      fakeAsync(() => {
        ...
        const hackerMonica = fixture.debugElement.queryAll(
          By.css('tr[app-hacker]')
        )[0].nativeElement;
        hackerMonica.click();

        expect(router.navigate).toHaveBeenCalledWith([
          '/hackers/70dd6f38-fd14-4dfd-bd43-3b07586ce49e'
        ]);
      })
    );
  });
